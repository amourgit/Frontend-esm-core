...
describe('runAssemble', () => {
  ...
  describe('survey mode', () => {
    it('fetches packages from npm registry and offers only -app packages in the checkbox', async () => {
      ...
      expect(mockNpmFetchJson).toHaveBeenCalledWith(expect.stringContaining('keywords:egen'), expect.any(Object));
  ...
  describe('output generation', () => {
    ...
    it('merges configFiles into egen-config.json', async () => {
      const config = { frontendModules: {}, publicUrl: '.' };
      mockExistsSync.mockReturnValue(true);
      mockReadFile.mockResolvedValue(JSON.stringify(config));
      ...
      const writeCall = mockWriteFile.mock.calls.find(([path]) => String(path).includes('egen-config'));
      expect(writeCall).toBeDefined();
