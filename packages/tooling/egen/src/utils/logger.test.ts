...
describe('logInfo', () => {
  it('calls console.log with the [egen] prefix and message', () => {
    const spy = vi.spyOn(console, 'log').mockImplementation(() => {});
    logInfo('test message');
    expect(spy).toHaveBeenCalledOnce();
    expect(spy.mock.calls[0]![0]).toContain('[egen]');
    expect(spy.mock.calls[0]![0]).toContain('test message');
  });
});

describe('logWarn', () => {
  it('calls console.warn with the [egen] prefix and message', () => {
...
describe('logFail', () => {
  it('calls console.error with the [egen] prefix and message', () => {
