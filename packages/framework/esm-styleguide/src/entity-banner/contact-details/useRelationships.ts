...
export function useRelationships(entityUuid: string) {
  const apiUrl = `${restBaseUrl}/relationship?v=${customRepresentation}&person=${entityUuid}`;

  const { data, error, isLoading, isValidating } = useSWR<{ data: RelationshipsResponse }, Error>(
    entityUuid ? apiUrl : null,
    egenFetch,
  );

  const formattedRelationships = data?.data?.results?.length
    ? extractRelationshipData(entityUuid, data.data.results)
    : null;
