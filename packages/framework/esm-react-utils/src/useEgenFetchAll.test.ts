...
import { useEgenFetchAll } from './useEgenFetchAll';
import { type EgenPaginatedResponse } from './useEgenPagination';
...
describe('useEgenFetchAll', () => {
  ...
  it('should render all rows on if number of rows > pageSize with no partialData', async () => {
    ...
    const { result } = renderHook(() =>
      useEgenFetchAll(`http://localhost/2`, {
        fetcher: (url) => getTestData(url, expectedRowCount).then((data) => ({ data }) as any),
      }),
    );
    await waitFor(() => expect(result.current.isLoading).toBeFalsy());
