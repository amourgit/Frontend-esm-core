...
import { type FetchConfig, egenFetch, type FetchResponse } from '@egen/esm-api';
import useAbortController from './useAbortController';

export type ArgumentsTuple = [any, ...unknown[]];
export type Key = string | ArgumentsTuple | undefined | null;
export type UseEgenSWROptions = {
...
function getUrl(key: Key, url?: string | ((key: Key) => string)): string {
  ...
  throw new Error(
    `When using useEgenSWR with a key that is not a string, you must provide a url() function that converts the key to a valid url. The key for this hook is ${key}.`,
  );
}
 ...
 *   to pass to {@link egenFetch} or options to pass to SWR
 */
export function useEgenSWR<DataType = any, ErrorType = any>(key: Key, options: UseEgenSWROptions = {}) {
  ...
  const fetcher = useCallback(
    (key: Key) => {
      ...
      return egenFetch(url_, { ...fetchInit, signal: abortSignal });
    },
