...
export interface EgenPaginatedResponse<T> {
  results: Array<T>;
 ...
 * @returns
 */
export function useEgenPagination<T>(
  url: string | URL,
  pageSize: number,
  options: UseServerPaginationOptions<EgenPaginatedResponse<T>> = {},
) {
  return useServerPagination<T, EgenPaginatedResponse<T>>(url, pageSize, egenServerPaginationHandlers, options);
}

type EgenServerPaginationHandlers<T> = ServerPaginationHandlers<T, EgenPaginatedResponse<T>>;
export const egenServerPaginationHandlers: EgenServerPaginationHandlers<any> = {
...
export function useServerPagination<T, R>(
  ...
  const fetcher: (key: string) => Promise<FetchResponse<R>> = options.fetcher ?? egenFetch;
