 ...
 */
export function useEgenInfinite<T>(
  url: string | URL,
  options: UseServerInfiniteOptions<EgenPaginatedResponse<T>> = {},
): UseServerInfiniteReturnObject<T, EgenPaginatedResponse<T>> {
  return useServerInfinite<T, EgenPaginatedResponse<T>>(url, egenServerPaginationHandlers, options);
}
