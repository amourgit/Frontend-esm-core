...
import { useEgenPagination, type EgenPaginatedResponse } from './useEgenPagination';
...
export async function getTestData(url: string, totalCount: number): Promise<EgenPaginatedResponse<number>> {
  ...
  return { results, links, totalCount } as EgenPaginatedResponse<number>;
}

describe('useEgenPagination', () => {
  it('should not fetch anything if url is null', async () => {
    const { result } = renderHook(() =>
      useEgenPagination(null as any, 50, {
  ...
  it('should fetch all rows on 1 page if number of rows < pageSize', async () => {
    ...
    const { result } = renderHook(() =>
      useEgenPagination('http://localhost/1', pageSize, {
  ...
  it('should fetch 2 pages if pageSize < number of rows <= 2 * pageSize', async () => {
    ...
    const { result } = renderHook(() =>
      useEgenPagination('http://localhost/2', pageSize, {
        fetcher: (url) => getTestData(url, expectedRowCount).then((data) => ({ data }) as any),
      }),
    );
  ...
  it('should fetch n pages for n >> 1', async () => {
    ...
    const { result } = renderHook(() =>
      useEgenPagination('http://localhost/3', pageSize, {
        fetcher: (url) => getTestData(url, expectedRowCount).then((data) => ({ data }) as any),
      }),
    );
